...
class DAMCNN(AutoTestTrainableModule):
    def __init__(self, config: DAMCNNConfig):
        super().__init__(config)
        self.cbam = CBAM(window_size=config.window_size, features=config.in_features)
        self.mscnn = MSCNN(window_size=config.window_size, features=config.in_features)
        ...
        self.dense = nn.Linear(32, 1) if config.output_layer_mode == "norm" else OutputLayer(32)
