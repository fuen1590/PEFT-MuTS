...
class IMDSSN(AutoTestTrainableModule):
    def __init__(self, config: IMDSSNConfig):
        super(IMDSSN, self).__init__(config)
        self.input_mapper = nn.Linear(in_features=config.in_features, out_features=config.hidden_dim)
        self.pe = PositionEmbedding(dim=config.hidden_dim,
                                    window_size=config.window_size,
                                    dropout=0,
                                    device=config.device) if config.pe else None
        self.in_features = config.in_features
        self.hidden_dim = config.hidden_dim
        self.MLSNEncoders = nn.Sequential()
        self.MPSNEncoders = nn.Sequential()
        for _ in range(config.encoder_nums):
            self.MLSNEncoders.append(Encoder(
                config.window_size, config.hidden_dim, attention=LogSparseAttention(
                    n_head=config.n_heads,
                    n_embd=config.hidden_dim,
                    win_len=config.window_size,
                    q_len=5,
                    sub_len=10,
                    scale=True,
                )
            ...
            self.MPSNEncoders.append(Encoder(
                config.window_size, config.hidden_dim, attention=ProbAttentionLayer(
                    n_heads=config.n_heads,
                    d_model=config.hidden_dim
                )
            ))
        self.fuse = nn.Linear(in_features=config.hidden_dim * 2,
                              out_features=config.hidden_dim,
                              bias=False)
        self.output = nn.Sequential(
            nn.Linear(in_features=config.window_size * config.hidden_dim, out_features=1)
        ) if config.output_layer_mode == "norm" else OutputLayer(config.window_size * config.hidden_dim)
